...
class Eval:
    def __init__(self, settings, index, bpp_savepath) -> None:
        ...
        self.seg_eval = EVAL_mAP(index, 'seg')
    ...
    def _feature_coding(self, inputs, fname_temp, nosave=False):
        time = time.time()
        ...
        out_criterion_p3 = self.criterion_p3(net_belle_output, d_p3, heigh_temp, width_temp, flag=3) #net_belle_output和d为pad后的
        out_criterion_p2 = self.criterion_p2(net_belle_output, d, heigh_temp, width_temp, flag=2)  # net_belle_output和d为pad后的
        out_criterion_p4 = self.criterion_p4(net_belle_output, d_p4, heigh_temp, width_temp, flag=4)  # net_belle_output和d为pad后的
        out_criterion_p5 = self.criterion_p5(net_belle_output, d_p5, heigh_temp, width_temp, flag=5)  # net_belle_output和d为pad后的
        ...
        # print('P4 bpp: %8.4f, MSE: %8.4f, PSNR: %8.4f' % (out_criterion_p4["bpp_loss"].item(), out_criterion_p4["mse_loss"].item(), psnr_temp_p4))

        define_mse = nn.MSELoss()
        out_criterion_p5["mse_loss"] = define_mse(d_output_p5, d_originalsize_p5)
        psnr_temp_p5 = mse2psnr(out_criterion_p5["mse_loss"])
        # print('P5 bpp: %8.4f, MSE: %8.4f, PSNR: %8.4f' % (out_criterion_p5["bpp_loss"].item(), out_criterion_p5["mse_loss"].item(), psnr_temp_p5))

        
        ######################### add compress method start #########################################
        set_entropy_coder("ans")
        self.model.net_belle.update(force=True)
        out_enc = self.model.net_belle.compress(d)
        out_dec = self.model.net_belle.decompress(out_enc["strings"], out_enc["shape"])
        define_mse = nn.MSELoss()
        ...
        p2_decompress_mse = define_mse(d_output_p2 ,d_originalsize_p2)
        p3_decompress_mse = define_mse(d_output_p3 ,d_originalsize_p3)
        p4_decompress_mse = define_mse(d_output_p4 ,d_originalsize_p4)
        p5_decompress_mse = define_mse(d_output_p5 ,d_originalsize_p5)
        num_pixels = self.height_temp * self.width_temp
        bpp = sum(len(s[0]) for s in out_enc["strings"]) * 8.0 / num_pixels


        print(fname_temp)
        print('estimation', mse2psnr(out_criterion_p2["mse_loss"].item()), mse2psnr(out_criterion_p3["mse_loss"].item()), mse2psnr(out_criterion_p4["mse_loss"].item()), mse2psnr(out_criterion_p5["mse_loss"].item()), out_criterion_p5["bpp_loss"].item())
        print('actual', mse2psnr(p2_decompress_mse.item()), mse2psnr(p3_decompress_mse.item()), mse2psnr(p4_decompress_mse.item()), mse2psnr(p5_decompress_mse.item()), bpp)


        ######################### add compress method end#########################################
        ...
        return features_p345

        


    def evaluation_nosave(self):
        with open(f"../result/output/{self.set_idx}_coco.txt", 'w') as of:
            ...
            with tqdm(total=len(self.data_loader.dataset)) as pbar:
                for inputs in iter(self.data_loader):
                    ...
                    features_SIMO = self._feature_coding(inputs, fname_temp, nosave=True)
        ...
        --input_predictions         ../result/output/{self.set_idx}_oi.txt \
        --output_metrics            ../result/output/{self.set_idx}_AP.txt"
        ...
        with open(os.path.join(os.environ["OUTPUT_DIR"], f"info/{self.set_idx}_coco.txt"), 'w') as of:
            ...
            with tqdm(total=len(self.data_loader.dataset)) as pbar:
                for inputs in iter(self.data_loader):
                    ...
                    guiyihua_max = self.numpixel_test5000[fname_temp][7]
                    guiyihua_min = self.numpixel_test5000[fname_temp][8]
                    guiyihua_scale = guiyihua_max - guiyihua_min
                    set_entropy_coder("ans")
                    self.model.net_belle.update(force=True)
